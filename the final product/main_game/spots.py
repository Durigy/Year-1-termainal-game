from .items import tools, food
from .map import rooms, roads


# Roads --------------------------------------------------------------------

# Herod Close
l_entrance_hc = {
    "name": roads["herod close"]["name"],

    "main description": "",
    
    "spot name": "Library Entrance",

    "description":
    """The entrance is passcode protected;
if you think you have the correct code - feel
free to try your luck. Of course...there's no
guarantee it will still work, after all these
years.
Going EAST will take you to the entrance
of Herod Close, and NORTH will allow you
to attempt access to the Library.""",

    "exits": {"east": "hc entrance", "north": "mystery aisle"},

    "tools": [],

    "items needed": ["paper passcode"],

    "comment": "",

    "food": [],

    "passcode": [],
    

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x                x | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |    [HERE]                   |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________| """
}

entrance_hc = {
    "name": roads["herod close"]["name"],

    "main description": roads["herod close"]["description"],

    "spot name": "HC Entrance",

    "description":
    """To your WEST is the front entrance
to the library, but it's password protected.
You decide it's not worth going up to it and
entering random numbers - you'll need something
that may give a clue to even attempt going over
there.
Alternatively, going SOUTH will take you to the
entrance of Laboratory 1 in Herod Street.""",
    
    "exits": {"west": "library entrance", "south": "lab 1 entrance"},

    "tools": [],

    "food": [],

    "items needed": [],

    "comment": "",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE  [HERE]  |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}

# Herod Street
lab1_entrance_hs = {
    "name": roads["herod street"]["name"],

    "main description": roads["herod street"]["description"],

    "spot name": "Lab 1 Entrance",

    "description":
    """You are standing in Herod Street,
facing Laboratory 1. Through the window you
can see that it's dirty and dingy - but what
needs to be clean is clean.
Going NORTH will take you to herod close,
WEST will take you into the laboratory and
SOUTH to the entrance of main street and the
carpark.""",

    "exits": {"north": "hc entrance", "south": "carpark main str entrance", "west": "lab1 main entrance"}, 

    "tools": [],

    "food": [],

    "items needed": [],

    "comment": "",

    "passcode": ["answer"],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  / [HERE] | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}

ms_cp_entrance_hs = {
    "name": roads["herod street"]["name"],

    "main description": roads["herod street"]["description"],

    "spot name": "Carpark Main Str Entrance",

    "description":
    """You are standing between the
entrance to the car park and the entrance to
Main Street.
Looking into the desolate car park, you can
see cars have begun to rust - unusable now that
necessities have been stripped from them.
To go into the carpark, go WEST. To go onto
Main Street, go EAST.""",

    "exits": {"east": "shop front", "west": "carpark entrance", "north": "lab 1 entrance"},

    "tools": [],

    "food": [],

    "items needed": [],

    "comment": "",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x / [HERE]                 x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}

# Main Street
front_shop = {
    "name": roads["main street"]["name"],

    "main description": roads["main street"]["description"],

    "spot name": "Shop Front",
    
    "description":
    """Standing in front of the shop,
you can see that the aisles are either
blocked or mostly empty within.
You can go SOUTH to enter the shop, go
EAST to go in front of The Plough or go
WEST to enter Herod Street.""",
    
    "exits": {"south": "shop entrance", "east": "pub front", "west": "carpark main str entrance"},

    "tools": [],

    "food": [],

    "items needed": [],

    "comment": "",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        ________________v       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                   [HERE]                              x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}

front_pub = {
    "name": roads["main street"]["name"],

    "main description": "",
    
    "spot name": "Pub Front",
    
    "description":
    """You're standing in front of The
Plough, which used to be creepy old men's
go to place when they can't be bothered
to argue with their wife.
To enter it, you go SOUTH. Going WEST
takes you to the front of the shop, and
EAST to infront of the alleyway.""",

    "exits": {"east": "alley front", "south": "pub centre", "west": "shop front"},

    "tools": [],

    "food": [],

    "items needed": [],

    "comment": "",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+--+------------+---------+------------+--------+------------+----|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                               [HERE]                x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}

front_alley = {
    "name": roads["main street"]["name"],

    "main description": roads["main street"]["description"],

    "spot name": "Alley Front",
    
    "description":
    """You're now on the corner of
Main Street, standing by a dark and gloomy
alley way that's probably always been as such,
and on the cusp of Isiah Road.
To enter the alleyway, go SOUTH. Or,
you can go NORTH onto Isiah Road or
WEST to the pub front.""",

    "exits": {"south": "alley entrance", "north": "ir centre", "west": "pub front"},

    "tools": [],

    "food": [],

    "items needed": [],

    "comment": "",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        ________________v       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                 [HERE] |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}

# Isiah Road
ir_centre = {
    "name": roads["isiah road"]["name"],

    "main description": roads["isiah road"]["description"],

    "spot name": "IR Centre",

    "description":
        """You're now standing in Isiah Road.
To your right (EAST) is Willow Park, where nature
and life has bloomed after it could no longer be
kept neat and tidy. A willow tree bends above a
pond.
To your SOUTH is Main Street, going NORTH will take
you to inbetween a windy path and in front of the
entrance to Laboratory 2.""",

    "exits": {"east": "park entrance", "north": "ir north centre", "south": "alley front"},

    "tools": [],

    "food": [],

    "items needed": [],

    "comment": "",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | | [HERE] /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}

north_ir_centre = {
    "name": roads["isiah road"]["name"],

    "main description": roads["isiah road"]["description"],

    "spot name": "IR North Centre",

    "description":
        """You're now at the north section of
Isiah Road. To your right is Laboratory 2. You can
see Dr. Gavigan within, on his wheelchair - he's
patiently waiting to make the vaccine, despite
his immiment demise.. You'll need an ID Card to
enter the lab, however.
To attempt entering Lab 2, go EAST. SOUTH will take
you down to where the park is, and WEST will take
you down a windy, cobblestone path.""",

    "exits": {"east": "lab2 main entrance", "south": "ir centre", "west": "w centre"},

    "tools": [],

    "food": [],

    "items needed": [],

    "comment": "",

    "passcode": ["answer"],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \          [HERE]  / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}

# Windy Path
windy_centre = {
    "name": roads["windy path"]["name"],

    "main description": roads["windy path"]["description"],

    "spot name": "W Centre",

    "description":
        """You notice an ID Card on the floor, which
will get you back into Laboratory 2. That's good!
You can either go NORTH, to investigate
the nest. EAST will take you to Isiah Road, SOUTH
to the box and WEST goes straight to the library.""",

    "exits": {"north": "birds nest", "east": "ir north centre", "south": "box", "west": "library side entrance"},

    "tools": ["id card"],

    "food": [],

    "items needed": [],

    "comment": "",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \  [HERE]   /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}


birds_nest = {
    "name": roads["windy path"]["name"],

    "main description": "",
    
    "spot name": "Birds Nest",

    "description":
        """The pigeon gives you a stern glare as
you approach it - but becomes skittish and flies to
another tree when you get to close. In her nest are
eggs and a vial, labelled 'Substance 2'. You wonder
if the pigeon is, in fact, a thief. At the base
of the tree, there is a pack of salted crips, untouched.
You wonder if crisps have an expiry date.
Going WEST will take you straight to the library,
EAST to Isiah Road and SOUTH back onto the cobblestone
path.""",

    "exits": {"west": "library side entrance", "east": "ir north centre", "south": "w centre"},

    "tools": ["substance2"],

    "food": ["salted crisps"],

    "items needed": [],

    "comment": "",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                            [HERE]                      |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}

spot_box = {
    "name": roads["windy path"]["name"],

    "main description": "",
    
    "spot name": "Box",

    "description":
        """You're now standing at fence of
the houses, looking up into the windows you
can see some refugees, looking down - the light
gone from their eyes. At the base of the fence
is a cardboard box, half-open. Inside, there
are a lot of beakers! These will come in handy.
Among them, there is also a can of beans.
To go back to the path, go NORTH. Going WEST will
take you straight to the library, and EAST will
take you back onto Isiah Road.""",

    "exits": {"north": "w centre", "east": "ir north centre", "west": "library side entrance"},

    "tools": ["beakers"],

    "food": ["canned beans"],

    "items needed": [],

    "comment": "",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                               [HERE]                                   |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}


# Rooms --------------------------------------------------------------------

# Car park
cp_lab1_side_entrance = {
    "name": rooms["car park"]["name"],

    "main description": "",
    
    "spot name": "CP Lab 1 Side Entrance",

    "description":
        """The side entrance to Lab 1, the door
is slowly rusting. Behind you, you notice a car with
the door wide open.
Go go inside Laboratory 1, go NORTH. To return to
the carpark entrance, go EAST. Going SOUTH  will
lead you to the car.""",

    "exits": {"east": "carpark entrance", "north": "lab 1 side entrance", "south": "car"},

    "tools": [],

    "food": [],

    "items needed": [],

    "comment": "",

    "passcode": ["answer"],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |[HERE]              |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}



carpark_entrance = {
    "name": rooms["car park"]["name"],

    "main description": "",
    
    "spot name": "Carpark Entrance",

    "description":
        """You are now by the entrance of
the car park, next to Herod Street. To go onto
Herod Street, go EAST. Going WEST will take
you to the side entrance of Laboratory 1.""",

    "exits": {"east": "carpark main str entrance", "west": "cp lab 1 side entrance"},

    "tools": [],

    "food": [],

    "items needed": [],

    "comment": "",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE     x    |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |             [HERE] /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}

spot_car = {
    "name": rooms["car park"]["name"],

    "main description": "",
    
    "spot name": "Car",

    "description":
    """Standing in front of the car, you
can see a bag on the passenger's seat.
Looking inside, there is a torch - without
batteries, however. By the pedal you notice a
can of new potatoes - you hope they'll still
be good enough to eat.
To go to the side entrance of Lab 1, go NORTH.
Going EAST will take you to the entrance of the
carpark.""",

    "exits": {"north": "cp lab 1 side entrance", "east": "carpark entrance"},
    
    "tools": ["torch"],

    "food": ["canned potatoes"],

    "items needed": [],

    "comment": "",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |  [HERE]            |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}

# Laboratory 1

lab1_main_entrance = {
    "name": rooms["laboratory 1"]["name"],

    "main description": "",
    
    "spot name": "Lab 1 Main Entrance",

    "description":
        """You're now in lab 1, by the main
entrance. To your NORTH is a desk, and to your
WEST is the side entrance - that leads to the
carpark. To go onto Herod Street, go EAST.""",

    "exits": {"north": "desk", "west": "lab 1 side entrance", "east": "lab 1 entrance"},

    "tools": [],

    "food": [],

    "items needed": [],

    "comment": "",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1  [HERE]/  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}

lab1_side_entrance = {
    "name": rooms["laboratory 1"]["name"],

    "main description": "",
    
    "spot name": "Lab 1 Side Entrance",

    "description":
    """You're now by the side entrance of the
laboratory. It's less rusted on the inside than
out, but it still manages to open. You wonder how
many times scientists and technicians of the past
made their way through here.
Leaving through the door, to the SOUTH, will take
you into the carpark. Going NORTH will take you
to the syringe cupboard, and EAST back to the main
entrance.""",

    "exits": {"south": "cp lab 1 side entrance", "north": "syringe cupboard", "east": "lab1 main entrance"},

    "tools": [],

    "food": [],

    "items needed": [],

    "comment": "",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |[HERE]              | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}

spot_desk = {
    "name": rooms["laboratory 1"]["name"],

    "main description": "",
    
    "spot name": "Desk",

    "description":
    """On top of the desk is a laptop, it's
low on charge but contains some important
formulas Dr. Gavigan has created. Next to it,
there is a bag of salted crisps.
Going WEST will take you to the syringe
cupboard, and SOUTH will take you to the main
entrance.""",

    "exits": {"west": "syringe cupboard", "south": "lab1 main entrance"},

    "tools": ["laptop"],

    "food": ["salted crisps"],

    "items needed": [],

    "comment": "",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x        [HERE] |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}

syringe_cupboard = {
    "name": rooms["laboratory 1"]["name"],

    "main description": "",
    
    "spot name": "Syringe Cupboard",

    "description":
    """The syringe cupboard has been left
wide open, but you first notice a can of
beans!
After taking a peak inside, you realise the syringe
cupboard has lived up to it's purpose; the
bag is full of empty syringes. You also discover,
next to the bag, there is a vial that is labelled
'Substance 1'. Seems like Dr. Gavigan left it in a
somewhat normal place, for once.
Going EAST will take you to the desk, and
SOUTH will take you to the side entrance.""",

    "exits": {"east": "desk", "south": "lab 1 side entrance"},
        
    "tools": ["syringes bag", "substance1"],

    "food": ["canned beans"],

    "items needed": [],

    "comment": "",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |   [HERE]        x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}


# Shop
shop_entrance = {
    "name": rooms["shop"]["name"],

    "main description": "",
    
    "spot name": "Shop Entrance",
    
    "description":
    """Only one of the aisles seem to
contain something of use. On the counter
top next to you, there's a singular clean
mask kept in a pack, from a pandemic a
decade ago. Beside it is an unopened can
of potatoes. 
To go to the cleaning aisle, go WEST.
Going NORTH will take you back onto Main Street.""",
    
    "exits": {"north": "shop front", "west": "cleaning aisle"},

    "tools": ["mask"],

    "food": ["canned potatoes"],

    "items needed": [],

    "comment": "",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |          [HERE]            |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}

cleaning_aisle = {
    "name": rooms["shop"]["name"],
    
    "main description": "",
    
    "spot name": "Cleaning Aisle",

    "description":
    """The cleaning aisle has the most
items - it's rather unlikely people would
want limescale remover in a zombie
apocalypse, after all. Sure enough, there's
a disinfectant product that would prove
useful.
You can only go EAST from here, back to
the shop entrance.""",

    "exits": {"east": "shop entrance"},

    "tools": ["cleaning product"],

    "food": [],

    "items needed": [],

    "comment": "",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           | [HERE]   SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}



# The Plough (Pub)
pub_centre = {
    "name": rooms["the plough"]["name"],

    "main description": "",
    
    "spot name": "Pub Centre",
    
    "description":
    """You can go NORTH onto Main Street,
go EAST to get behind the bar or go
SOUTH to leave into the alleyway.""",
    
    "exits": {"north": "pub front", "east": "behind bar", "south": "pub back entrance"},

    "tools": [],

    "food": [],

    "items needed": [],

    "comment": "",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB [HERE]           |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}

behind_bar = {
    "name": rooms["the plough"]["name"],

    "main description": "",
    
    "spot name": "Behind Bar",
    
    "description":
    """The bar has been scrounged for
liquor, people wanting a break from reality.
To your luck, and surprise, there is an
untouched bottle of Vodka. On top of the
bar, you also spot a beer-stained piece
of paper with a passcode on it; you have
a sneaking suspicion it may come in handy
for the library.""",

    "exits": {"west": "pub centre"},

    "tools": ["paper passcode"],

    "food": ["vodka"],

    "items needed": [],

    "comment": "",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                       [HERE]|         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}

# Alleyway
alley_entrance = {
    "name": rooms["alleyway"]["name"],

    "main description": "",
    
    "spot name": "Alleyway Entrance",

    "description":
    """You stand on the corner of the alley,
next to the light of Main Street.
To go onto Main Street, go NORTH. Going
WEST will take you to the back entrance of
the pub - beside it is a dog kennel.""",

    "exits": {"north": "alley front", "west": "pub back entrance"},
    
    "tools": [],

    "food": [],

    "items needed": [],

    "comment": "",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                             [HERE] |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}

pub_back_entrance = {
    "name": rooms["alleyway"]["name"],

    "main description": "",
    
    "spot name": "Pub Back Entrance",

    "description":
    """The back door to the pub is broken,
the top coming off of it's hinges. Beside the
door is a dogs kennel - and within that is
an axe. It was probably used in ridding of
the undead that once lived in the alleyway.
To enter the pub, go NORTH. To go to the
entrance of the alley, go EAST.""",

    "exits": {"north": "pub centre", "east": "alley entrance"},

    "tools": ["axe"],

    "food": [],

    "items needed": [],

    "comment": "",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                            [HERE]                     |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}


# Laboratory 2
lab2_main_entrance = {
    "name": rooms["laboratory 2"]["name"],

    "main description": "",
    
    "spot name": "Lab 2 Main Entrance",
    
    "description":
    """Dr. Gavigan sits in his wheelchair,
looking especially frail today. 'Have you
got anything for me? I need these items!:
Substance 1, 
Substance 2, 
Substance 3, 
Laptop, 
Beakers, 
Cleaning Products and a 
Syringe Bag. 
To go to the tabletop, go EAST. To go back
onto Isiah Road, go WEST.""",

    "exits": {"east": "tabletop", "west": "ir north centre"},

    "tools": [],

    "food": [],

    "items needed": ["id card"],

    "comment": "To be in Lab 2 you need an ID Card.",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    /[HERE]            x | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}

spot_tabletop = {
    "name": rooms["laboratory 2"]["name"],

    "main description": "",
    
    "spot name": "Tabletop",

    "description":
    """In front of you lays an open tabletop,
where Dr. Gavigan expects you to drop items
that he'll need. For you to take, there is a
chocolate bar.
To go back to the entrance of the laboratory,
go WEST.""",

    "exits": {"west": "lab2 main entrance"},

    "tools": [],

    "food": ["chocolate bar"],

    "items needed": ["id card"],

    "comment": "To be in Lab 2 you need an ID Card.",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x            [HERE]| *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}

# Contaminated Area
contam_area = {
    "name": rooms["contaminated area"]["name"],
    
    "main description": rooms["contaminated area"]["description"],

    "spot name": "Contaminated Area Centre",

    "description":
    """On the floor, by the door, there is another
unopened jar of pickles. You wonder if it's
Dr. Gavigan's favourite food item.
To leave the room, go SOUTH.""",

    "exits": {"south": "p lab2 side entrance"},

    "tools": [],

    "food": ["pickles"],

    "items needed": ["mask"], 

    "comment": "Keys, wallets, phone...ah sh*t, my mask!!",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x                x | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           | [HERE] | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}

# Willow Park
park_entrance = {
    "name": rooms["willow park"]["name"],

    "main description": "",
    
    "spot name": "Park Entrance",
    
    "description":
    """From here, the entrance to the contaminated
area is blocked off. On top of a bin next to you,
you find a pack of batteries - they'll work for a
torch.
You can go EAST to the Willow tree, or return WEST
onto Isiah Road.""",
    
    "exits": {"east": "willow tree", "west": "ir centre"},

    "tools": ["batteries"],

    "food": [],

    "items needed": [],

    "comment": "",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    / [HERE]            |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}

willow_tree = {
    "name": rooms["willow park"]["name"],

    "main description": "",
    
    "spot name": "Willow Tree",
    
    "description":
    """The willow tree twists over the pond,
some of the branches brush the water - like
a greeting. It looks like it was standing
here for a while - but it also looks tired.
Beneath it, you find a golden pocket watch.
Opening it, you can see a picture of
Dr Gavigan and his late wife. You leave it
where you found it. Beside the watch is an
unopened jar of pickles.
Looking up, you can just see over the mound
of boxes and barriers the door to the
contaminated area. Perhaps, with an axe,
you can make it through there, to investigate.
From here, you can attempt going NORTH to
the contaminated room entrance, or WEST to
return to the park entrance.""",

    "exits": {"north": "p lab2 side entrance", "west": "park entrance"},
    
    "tools": [],

    "food": ["pickles"],

    "items needed": [],

    "comment": "",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       /HERE|  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}

# To access here, you need an axe to break down the barrier.
p_lab2_side_entrance = {
    "name": rooms["willow park"]["name"],

    "main description": "",
    
    "spot name": "P Lab 2 Side Entrance",
    
    "description":
    """You've managed to get behind the barrier,
but you still need a mask to get in the contaminated
area. The windows have been blacked out; you wonder,
and fear, what's inside. Thinking about it, it's not
a bad idea to have a mask on either, if it truly
contaminated...
Going NORTH will take you inside the building, but
you must have a mask on you. SOUTH will take you back
to the Willow Tree.""",
    
    "exits": {"north": "contam area centre", "south": "willow tree"},

    "tools": [],

    "food": [],

    "items needed": ["axe"],

    "comment": "",

    "passcode": ["answer"],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |             [HERE]|  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}

# Library
l_side_entrance = {
    "name": rooms["library"]["name"],

    
    "main description": "",
    

    "spot name": "Library Side Entrance",

    "description":
    """You're now standing by the entrance
of the library - drops of water drip on your
face from the tiles above. In the corner is
the only aisle that you can access, but it's
shrouded in darkness. You'll need a torch
to be able to see there - but remember that
batteries are not included.
Going EAST will take you to the windy
path and WEST to the mystery aisle.""",
    
    "exits": {"east": "w centre", "west": "mystery aisle"},

    "tools": [],

    "food": [],

    "items needed": [],

    "comment": "",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY  [HERE] /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |       x              |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}


# You need a torch (and batteries) in your inventory to light up this.
mystery_aisle = {
    "name": rooms["library"]["name"],

    "main description": "",
    
    "spot name": "Mystery Aisle",

    "description":
    """The mystery aisle is shrouded in darkness,
but your torch gives it light. Searching the shelves,
you manage to find a rather dusty vial - labelled
'Substance 3'. There's a also a chocolate bar - you
hope that it still tastes okay. The wrapper is
covered in dust, there's even a small spider
crawling across it, which only makes your spine shiver.
You can go EAST to go back to the side
entrance of the library. You can open the door without
a passcode from the inside, which means you can go
SOUTH onto Herod Close.""",

    "exits": {"east": "library side entrance", "south": "library entrance"},

    "tools": ["substance3"],

    "food": ["chocolate bar"],

    "items needed": ["torch", "batteries"],

    "comment": "To be in the mystery isle you need a torch and batteries to power it.",

    "passcode": [],

    "map": """_____________________________________________________________________________________________________________________________________
|                                                                                                                                   |
|***********************************************************************************************************************************|
|* +----------------------+                                                                         ________+--------------------+ *|
|* |                      |                                               x                        |        |                    | *|
|* |                      |___     ____________        _________________       _____________       |        |                    | *|
|* |                      /   \   /            \      /   WINDY PATH    \     /             \      |        |                    | *|
|* |      LIBRARY      x  /    \_/   _______    \____/    ___________    \___/    _______    \_____|        |        LAB 2       | *|
|* |                      |\        /       \            /           \    x      /       \             x    / x               x  | *|
|* |                      | \______/         \__________/             \_________/         \________         /                    | *|
|* |                      |                                                                        |        |            --------| *|
|* |     [HERE]           |                                   x                                    |        |           |   x    | *|
|* +------/\/-------------+------+---+------------+---------+------------+--------+------------+---|        |           |        | *|
|* |       x                     |   |            |         |            |        |            |   |        +--------------/\/---+ *|
|* |        HEROD CLOSE    x     |   |            |         |            |        |            |   |        |               x   |  *|
|* |                             |   |   GARDEN   |         |   GARDEN   |        |   GARDEN   |   | ISIAH  |                   |  *|
|* +--------------------+        |   |            |         |            |        |            |   | ROAD   |        PARK       |  *|
|* |    x            x  |        | +-+------------+-+     +-+------------+-+    +-+------------+-+ |        /                   |  *|
|* |                    |        | |                |     |                |    |                | |   x    /   x               |  *|
|* |       LAB 1     x  /  x     | |     EMPTY      |     |     EMPTY      |    |     EMPTY      | |        |         _____     |  *|
|* |                    /        | |     HOUSE      |     |     HOUSE      |    |     HOUSE      | |        |      _/       \   |  *|
|* |                    | HEROD  | |                |     |                |    |                | |        |     /          \  |  *|
|* |  x                 | STREET | +----------------+     +----------------+    +----------------+ |        |    /   POND    /  |  *|
|* +-/ /----------------+        |_________________________________________________________________|        |    \          /   |  *|
|* |  x                 |                                                                                   |     \        |    |  *|
|* |                    |                                     MAIN STREET                                   |     /         \   |  *|
|* |                  x /  x                     x                                 x                   x    |     |         /   |  *|
|* |      CARPARK       /           +-----------/ /--------------+__+-------------/ /-------------+         |      \       / x  |  *|
|* |                    |           |            x               |  |                           x |         |        \___/      |  *|
|* |                    |           |   x      SHOP              |  |        PUB   x              |         |                   |  *|
|* |                    |           |                            |  |                             |         |                   |  *|
|* |    x               |           +----------------------------+  +-------------/ /-------------+         +-------------------+  *|
|* +--------------------+           |                                              x                        |                      *|
|*                      |           |                           ALLEYWAY                               x    |                      *|
|*                      |           |                                                                       |                      *|
|*                      |___________|_______________________________________________________________________|                      *|
|*                                                                                                                                 *|
|***********************************************************************************************************************************|
|___________________________________________________________________________________________________________________________________|"""
}



# References --------------------------------------------------------------------

spots = {
    # Herod Close
    "library entrance": l_entrance_hc,
    "hc entrance": entrance_hc,
    # herod Street
    "lab 1 entrance": lab1_entrance_hs,
    "carpark main str entrance": ms_cp_entrance_hs,
    # Main Street
    "alley front": front_alley,
    "shop front": front_shop,
    "pub front": front_pub,
    # Isiah Road
    "ir centre": ir_centre,
    "ir north centre": north_ir_centre,
    # Windy Path
    "w centre": windy_centre,
    "birds nest": birds_nest,
    "box": spot_box,

    # Car park
    "cp lab 1 side entrance": cp_lab1_side_entrance,
    "carpark entrance": carpark_entrance,
    "car": spot_car,
    # Lab 1
    "lab1 main entrance": lab1_main_entrance,
    "desk": spot_desk,
    "syringe cupboard": syringe_cupboard,
    "lab 1 side entrance": lab1_side_entrance,
    # Shop
    "shop entrance": shop_entrance,
    "cleaning aisle": cleaning_aisle,
    # Pub
    "pub centre": pub_centre,
    "behind bar": behind_bar,
    # Alleyway
    "alley entrance": alley_entrance,
    "pub back entrance": pub_back_entrance,
    # Lab 2
    "lab2 main entrance": lab2_main_entrance,
    "tabletop": spot_tabletop,
    # Contaminated Area
    "contam area centre": contam_area,
    # Park
    "park entrance": park_entrance,
    "willow tree": willow_tree,
    "p lab2 side entrance": p_lab2_side_entrance,
    # Library
    "library side entrance": l_side_entrance,
    "mystery aisle": mystery_aisle
}


